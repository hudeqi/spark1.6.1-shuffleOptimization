...
private[spark] trait Spillable[C] extends Logging {
  ...
  // Number of elements read from input since last spp
  ...
  protected def maybeSpill(collection: C, currentMemory: Long): Boolean = {
    ...
    // Get the Map that stores the task Id to the spilling times
    val spillTimes = UnifiedMemoryManager.spillTimes
    //Get the task Id of Currently spill
    val curTaskId = taskMemoryManager.getCurTaskId

    // Actually spill
    if (shouldSpill) {
      _spillCount += 1
      //Update the spilling times of the task
      if (spillTimes.size != 0){
        spillTimes(curTaskId) = _spillCount
      }
