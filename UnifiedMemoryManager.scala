 ...
 */
private[spark] class UnifiedMemoryManager private[spark] (
    conf: SparkConf,
    val maxMemory: Long,
    val allocateMode: String,
...
//  override def spillTimes: mutable.HashMap[Long, Long] = synchronized{
  ...
  override private[memory] def acquireExecutionMemory(
    ...
    memoryMode match {
      case MemoryMode.ON_HEAP =>
        ...
        // Select which memory allocation algorithm to call based on the parameter allocateMode
        if (allocateMode == "FAIR") // Fair Allocation algorithm
          onHeapExecutionMemoryPool.acquireMemory(
          numBytes, taskAttemptId, maybeGrowExecutionPool, computeMaxExecutionPoolSize)
        else  // Adaptive Allocation Based Spilling times algorithm
          onHeapExecutionMemoryPool.acquireMemoryWithSpillTimes(
            numBytes, taskAttemptId, maybeGrowExecutionPool, computeMaxExecutionPoolSize, UnifiedMemoryManager.spillTimes)
...
object UnifiedMemoryManager {
  ...
  def apply(conf: SparkConf, numCores: Int): UnifiedMemoryManager = {
    ...
    val allocateMode = getAllocateCate(conf)
  ...
  // Determine which Execution Memory allocation to chose based on the running
  // parameter "spark.shuffle.allocate.mode".
  private def getAllocateCate(conf: SparkConf): String = {
    val allocate = conf.get("spark.shuffle.allocate.mode", "FAIR")
    allocate
  }

  // The data structure that stores the task Id  map to its spilling times.
  val spillTimes = new mutable.HashMap[Long, Long]()
}